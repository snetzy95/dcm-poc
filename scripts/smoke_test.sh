...
INSTANCE_ID=$(echo "$UPLOAD_RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['ID'])")
pass "Uploaded instance: $INSTANCE_ID"

ORTHANC_STUDY_ID=$(curl -sf "${ORTHANC_URL}/instances/${INSTANCE_ID}" | \
...
COUNT=$(echo "$STUDIES" | python3 -c "import sys,json; print(json.load(sys.stdin)['total'])")
[[ "$COUNT" -ge 1 ]] && pass "Core service has $COUNT study(ies)" || fail "Core service has 0 studies"

STUDY_UID=$(echo "$STUDIES" | python3 -c "import sys,json; print(json.load(sys.stdin)['items'][0]['study_uid'])")
...
COHORT_ID=$(echo "$COHORT_RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['cohort_definition_id'])")
...
MATCHED=$(echo "$RESOLVE" | python3 -c "import sys,json; print(json.load(sys.stdin)['matched_count'])")
...
GLOBAL_LOSS=$(echo "$AGG" | python3 -c "import sys,json; print(json.load(sys.stdin).get('global_loss','null'))")
...
DEL_AT=$(echo "$STUDY_RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('deleted_at') or 'null')")
