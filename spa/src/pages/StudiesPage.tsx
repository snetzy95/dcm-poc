...
export default function StudiesPage() {
  ...
  const inputCls = 'border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 hover:border-slate-400 transition-all duration-200'
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-card border border-slate-100 p-6">
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            <label className="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Modality</label>
            <input className={inputCls} value={modality} onChange={e => { setModality(e.target.value); setPage(1) }} placeholder="CT" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Sex</label>
      ...
      {data && (
        <>
          <p className="text-sm text-slate-500">{data.total} total studies</p>
          {/* Table Card */}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium bg-white text-slate-700 border border-slate-300 rounded-lg hover:bg-slate-50 hover:border-slate-400 transition-all duration-200 disabled:opacity-40"
            >
              Prev
            </button>
